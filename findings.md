...
### 21.13 Batch 4: durable "Run now", and an upgrade test for the migrations (F-06, F-17)

**Run now (F-06).** The Settings button and the "Run verification" button called a route that ran the whole job *inside the web request*. Discovery or a deep benchmark can take minutes, so the browser waited with the connection open, a dropped connection left you not knowing what had happened, the job ran in the web process rather than the worker that owns scheduling, and the Settings UI painted "SUCCEEDED" and "Saved." the moment it started. Now the route records a request (`automation_jobs.run_requested_at`, migration 0015, two nullable columns) and returns **202** at once; the worker's next tick starts it under the same lease and reporting as a scheduled run, recorded as a manual run; the request clears in the same update that marks the job RUNNING, so a finished-looking job has always really run. Asking twice before it starts is one run; a request made while the job is already running waits and is not lost; it works even if the job's schedule is disabled. If no worker is running the response says so ("Queued, but no worker is running — it will start as soon as one is") instead of hanging. Both buttons now follow the real job state (queued → running → succeeded/failed, with the job's own summary) through one tested helper, and the jobs API returns each job's latest run.

**Upgrade test (F-17).** A fresh-database test cannot catch the real deployment risk: a migration that fails on old data, which stops the web container from starting. A new test brings a database to the previous release's schema (through 0011), plants rows shaped like the production audit's (inverted timestamps, zero token limits, status-0 smoke tests, a credential holding a partial key), applies every remaining migration as `pnpm db:migrate` does, and checks the rows are repaired, the constraints are NOT VALID yet enforce new writes, and a second run is a no-op. Removing a repair from 0014 makes it fail.
