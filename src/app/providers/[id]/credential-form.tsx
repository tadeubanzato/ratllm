...
export function CredentialForm({providerId,defaultEnv,slug,defaultConfig,configured}:{providerId:string;defaultEnv:string;slug?:string;defaultConfig?:Record<string,string>;configured?:boolean}) {
  ...
  return <form onSubmit={submit} className="settings-form"><label>Environment reference<input className="input" name="environmentVariable" defaultValue={defaultEnv} required pattern="[A-Z][A-Z0-9_]*"/></label><label>API key<input className="input" name="apiKey" type="password" autoComplete="new-password" placeholder={configured?MASKED_SECRET_PLACEHOLDER:"Enter API key"} minLength={8}/><small>Stored encrypted.{configured?" Leave blank to keep the current one.":" Leave blank only when the reference exists on the server."}</small></label>{extraFields.map(field=><label key={field.key}>{field.label}<input className="input" name={`config.${field.key}`} defaultValue={defaultConfig?.[field.key]??""} placeholder={field.placeholder}/></label>)}<div className="settings-actions"><button className="button primary" type="submit" disabled={busy}>Save credential</button><button className="button" type="button" onClick={verify} disabled={busy}>Test credential</button></div>{message&&<p role="status" className="settings-feedback">{message}</p>}</form>;
}
