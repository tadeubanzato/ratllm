    ...
    {active === "Free Model Sources" && <Card title="Free model sources" aside={<button className="button primary" type="button" onClick={() => setSourceModal({mode: "add"})}>Add source</button>}>
      ...
      <div className="settings-table-wrap"><table className="data-table settings-table"><thead><tr><th>Tier</th><th>Source</th><th>Type</th><th>Enabled</th><th>Status</th><th>Last sync</th><th>Yield</th><th>History</th><th></th></tr></thead><tbody>
        {[...sources].sort((a, b) => (tierRank[a.tier ?? "C"] ?? 4) - (tierRank[b.tier ?? "C"] ?? 4)).map(source => <tr key={source.id}>
          ...
          <td><StatusPill value={source.status}/>{source.status==="HEALTHY"&&<><br/><small>{source.discoveredModelCount.toLocaleString()} models</small></>}{(source.status==="DEGRADED"||source.status==="FAILED"||source.status==="BLOCKED")&&<><br/><small title={source.lastError??undefined} style={{display:"block",minWidth:220,maxWidth:300,whiteSpace:"normal"}}>{source.lastError?source.lastError.length>140?`${source.lastError.slice(0,140)}…`:source.lastError:source.status==="BLOCKED"?"Waiting for a credential":"No reason recorded"}</small></>}</td>
