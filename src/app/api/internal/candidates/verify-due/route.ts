...
export async function POST(request:Request){const id=correlationId(request);const supplied=request.headers.get("x-internal-api-secret")??request.headers.get("x-okame-internal-secret")??request.headers.get("authorization")?.replace(/^Bearer\s+/i,"");if(!env.INTERNAL_API_SECRET||supplied!==env.INTERNAL_API_SECRET)return apiError("UNAUTHORIZED","Valid internal API secret required",401,id);try{const discovery=await runDiscovery();return NextResponse.json({discovery,verification:await verifyDueCandidates(),correlationId:id})}catch(error){return apiError("CANDIDATE_VERIFICATION_FAILED",error instanceof Error?error.message:"Candidate verification failed",502,id)}}
