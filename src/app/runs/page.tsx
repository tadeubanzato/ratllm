...
export default async function RunsPage({searchParams}: {searchParams: Promise<{type?: string; page?: string}>}) {
  ...
  return <PageShell title="Runs" eyebrow={type ? `Execution history · filtered to ${type.replaceAll("_"," ")} · ${total} total` : `Execution history · ${total} total`} actions={type ? <Link className="button" href="/runs">Clear filter</Link> : undefined}>
