...
export function Sidebar() {
  ...
  return <aside className="sidebar">
    ...
    <nav>{navItems.map(([label, href, Icon]) => <Link key={href} href={href} className={cn(pathname === href || href !== "/" && pathname.startsWith(href) ? "active" : "")}><Icon size={16}/><span>{label}</span>{label === "Change Plans" && <b>2</b>}</Link>)}</nav>
