 ...
 */
export function candidateOnlyBlockReason(candidate: { source: string; evidence?: Record<string, unknown> | null }): string | null {
  if (!candidateOnlySources.has(candidate.source)) return null;
  ...
  const corroboratedByAuthority = corroborators.some(item => typeof item?.source === "string" && !candidateOnlySources.has(item.source));
