...
export const demoLanes: LaneSummary[] = ["smart-general","smart-coding","smart-agent","smart-deep","smart-long","smart-vision","smart-summary","smart-speech"].map((slug, index) => ({ id: slug, slug, name: slug.replace("smart-", "Smart ").replace(/^./, c => c.toUpperCase()), healthy: [3,2,2,2,2,1,2,0][index], total: [3,3,2,2,2,2,2,0][index], minimumHealthy: index === 7 ? 1 : 2, confidence: index === 7 ? "UNKNOWN" : index > 4 ? "MEDIUM" : "HIGH" }));
