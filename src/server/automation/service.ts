...
export async function runAutomation(type:AutomationType, trigger="SCHEDULED", options?:AutomationOptions){
  ...
  try {const summary=await execute(type,options);const finished=new Date();const durationMs=finished.getTime()-started.getTime();if(run)await db.update(syncRuns).set({status:"SUCCEEDED",finishedAt:finished,summary:{trigger,result:summary},updatedAt:finished}).where(eq(syncRuns.id,run.id));await db.update(automationJobs).set({status:"SUCCEEDED",lastRunAt:started,nextRunAt:nextCron((await db.select({schedule:automationJobs.schedule}).from(automationJobs).where(eq(automationJobs.type,type)).limit(1))[0].schedule,finished),durationMs,failureCount:0,lastError:null,updatedAt:finished}).where(eq(automationJobs.type,type));return {started:true,runId:run?.id??null,summary};
