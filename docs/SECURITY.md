...
The UI reports whether a credential is stored; it never returns a key, any part of one, or its ciphertext. Automation (health-based removal, promotion, reconciliation) only ever changes LiteLLM deployments that RatLLM manages. Changing any other deployment — including adopting it — is an explicit operator action, confirmed by typing the start of that deployment's own LiteLLM ID, and is written to the audit trail.
