...
| I14 | **Provider call budget.** RatLLM's own calls to one provider are capped per rolling 24 hours (`providers/call-budget-policy.ts`): OpenRouter 24 (its free models share 50 a day with real traffic), self-hosted unlimited, others 200. Candidate checks and health probes are limited **separately, each by its own usage**, so neither can starve the other: candidate checks stop at 60% of the budget, health probes at 100%. (Counting them together left a provider with many live deployments unable to ever prove a new model.) Candidates already on a pass streak are checked first. |
...
| I21 | **A candidate matches its deployment by the model, not the routing.** LiteLLM stores `<routing>/<model id>` (`openai/qwen-flash`). Comparing a deployment with a candidate uses `deploymentModelKey`, which drops the routing part; `bareModelKey` alone kept `openai/` for a name without a digit, so `qwen-flash`, `qwen-max`, `codestral-latest` never matched their own deployment: the page offered "Add to LiteLLM" for live models and automation could not tell they were live. The R flag shows only for a live deployment. |
| I22 | **A model that cannot stream is not dead.** The health probe streams; if the model answers HTTP 400 "does not support streaming" (Groq's text-classification models), it is asked once more without streaming and judged on that. They were being auto-removed after five such probes while answering every ordinary request. |
| I23 | **Non-chat override.** A model classified as not a chat model (a safety classifier, speech, embedding) is never added automatically. A person may add one that demonstrably answers as a **direct alias only** (`allowNonChat`, needs `directAlias`, refuses lanes and `trigger: auto`); it is never put in a lane, where it would answer real traffic with scores. RatLLM then monitors it like any other. |
| I24 | **Specialised models are not general chat models.** OCR / document-reading models (`typhoon-ocr`, `deepseek-ocr`) and machine-translation models (`qwen-mt-plus`, `hunyuan-mt`) answer a chat request, so they pass direct checks, but they only do one job. They are classified as non-chat (never auto-added to a lane) and can be added by hand as a direct alias (I23). |
| I25 | **A new deployment is judged by its own probe.** After adding, promotion smoke-tests the new deployment by its router id, never the lane alias: a lane holds up to twenty members and LiteLLM picks one, so a slow or broken neighbour was recorded as the new model failing. |
| I26 | **The status panel reports what is still wrong.** A failed run is reported only until a later run of the same kind succeeds (for a promotion, until that same model is added). A credential is "unverified" only where the provider has a credential-test endpoint; providers without one (Kilo, Sarvam) can never be verified. Messages name the providers. |
| I27 | **Provider key links.** The provider page links to where a key is made: the curated key page or console, else the page of the provider it is another spelling of (`vertex` -> `vertex-ai`), else the provider's own docs as a discovery source published it, labelled "Open provider docs". Only plain http(s) addresses are ever rendered from a source. Credential autodetect uses the exact curated link only. |
| I28 | **The Performance page is the router, probed.** (Formerly "Benchmarks"; `/benchmarks` redirects.) One row per deployment still in LiteLLM, each with its OWN latest probe, however old: taken per deployment from the database, never from a slice of the fleet's latest N probes (which showed a deployment with a full history as "NOT RUN, last tested never"). A deployment not yet probed shows "NOT RUN" and is counted as awaiting its first probe. Coverage is "probed in the last 24 hours". It measures availability and speed of a one-word reply, not answer quality. Like the LiteLLM page it compares itself with the live router on every load and says so when it differs, so it is either right or says it may not be. |
