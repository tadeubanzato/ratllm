...
- **`reconcile.ts`** — `reconcileLaneMembership()`, run by the `LANE_RECONCILE` automation hourly (slowed from an original 15-minute default — it's mostly a no-op). Re-adds any lane member whose router deployment went missing and re-pushes the fallback chains. Never removes a pinned (user-selected) assignment.

See `docs/FREE-MODEL-LIFECYCLE.md` for the full discover → verify → promote → monitor → remove loop and the automation schedule as a whole.
